...
from config import INSTAGRAM_APP_ID, INSTAGRAM_APP_SECRET, REDIRECT_URI
...
def exchange_code(code: str) -> tuple[str | None, str | None]:
    ...
    if not r.ok:
        print(f"[AUTH] Code exchange error: {r.text}")
        return None, None
    data = r.json()
...
def exchange_long_lived(short_token: str) -> str | None:
    ...
    if not r.ok:
        print(f"[AUTH] Long-lived exchange error: {r.text}")
        return None
...
def fetch_recent_media(user_id: str, token: str) -> list[dict]:
    ...
    if not r.ok:
        print(f"[POLL] API error for user {user_id}: {r.text}")
        return []
