...
def get_all_users(platform: str) -> list[dict]:
    with _conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                "SELECT user_id, username, token, last_media_id FROM users WHERE platform = %s",
                (platform,)
            )
            return [dict(row) for row in cur.fetchall()]


def upsert_user(platform: str, user_id: str, username: str, token: str, last_media_id: str | None):
    with _conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO users (platform, user_id, username, token, last_media_id, updated_at)
                ...
                SET username = EXCLUDED.username,
                    ...
                    updated_at = NOW()
            """, (platform, user_id, username, token, last_media_id))
